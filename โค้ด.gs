...
function getAllStudents() {
  ...
  // อ่านถึงคอลัมน์ 17 (เพิ่มส่วน Choice)
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    students.push({
      id: row[0], exam_id: String(row[1]).replace(/^'/, ''), full_name: row[2], previous_school: row[3], grade_level: row[4],
      thai_score: parseFloat(row[5])||0, math_score: parseFloat(row[6])||0, science_score: parseFloat(row[7])||0, english_score: parseFloat(row[8])||0, aptitude_score: parseFloat(row[9])||0, total_score: parseFloat(row[10])||0, rank: parseInt(row[11])||0, national_id: String(row[12]||'').replace(/^'/, ''),
      // เพิ่มบรรทัดนี้ เพื่อส่งข้อมูลโครงการกลับไป
      choice_1: row[13]||'', choice_2: row[14]||'', choice_3: row[15]||'', admission_result: row[16]||''
    });
  }
  return { success: true, students: students };
}

// Create/Update ต้องรับ Choice ด้วย
function createStudent(data) {
  const sheet = getSheet(); const id = Utilities.getUuid();
  const newRow = [id, "'" + data.exam_id, data.full_name, data.previous_school, data.grade_level, data.thai_score||0, data.math_score||0, data.science_score||0, data.english_score||0, data.aptitude_score||0, data.total_score||0, data.rank||0, "'" + (data.national_id||''), data.choice_1||'', data.choice_2||'', data.choice_3||'', data.admission_result||''];
...
function createStudentsBulk(students) {
  ...
  sheet.getRange(sheet.getLastRow() + 1, 1, newRows.length, 17).setValues(newRows); return { success: true, message: `นำเข้าสำเร็จ ${newRows.length} รายการ` };
}
...
function updateStudent(data) {
  ...
  for (let i = 1; i < allData.length; i++) {
    if (allData[i][0] === data.id) {
      sheet.getRange(i + 1, 1, 1, 17).setValues([[data.id, "'" + data.exam_id, data.full_name, data.previous_school, data.grade_level, data.thai_score||0, data.math_score||0, data.science_score||0, data.english_score||0, data.aptitude_score||0, data.total_score||0, data.rank||0, "'" + (data.national_id||''), data.choice_1||'', data.choice_2||'', data.choice_3||'', data.admission_result||'']]);
